...
1. 理解并掌握文件 IO 相关API的用法。
2. 理解并掌握Linux 系统中文件目录的操作和用途。
3. 理解并掌握 Linux 系统中链接的相关操作。

## 2 实验内容

- 文件IO相关API
- 查看指定目录下所有文件的名称
- 创建文件的硬链接与软链接，查看链接文件的索引节点号和文件内容
...
int main(void){
    int fd;
    fd = open("/home/loheagn/no_exist_file",O_WRONLY);
    if(fd < 0){
        perror("/home/loheagn/no_exist_file");
        return 0;
    }
}
```
...
```shell
loheagn@ubuntu:~/splab$ gcc -o perrortest perrortest.c
loheagn@ubuntu:~/splab$ ./perrortest
/home/loheagn/no_exist_file: No such file or directory # 打印出了错误信息：没有这个文件/目录
```
